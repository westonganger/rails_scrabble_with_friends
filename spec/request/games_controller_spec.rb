...
RSpec.describe ScrabbleWithFriends::GamesController, type: :request do
  ...
  context "send_turn_reminder" do
    ...
    it "returns 404 when web push not configured" do
      ...
      post scrabble_with_friends.send_turn_reminder_game_path(@game)
      expect(response.status).to eq(404)
    end
    ...
    it "does not send reminder if no subscriptions" do
      ...
      post scrabble_with_friends.send_turn_reminder_game_path(@game)
      expect(response).to redirect_to(scrabble_with_friends.game_path(@game))
      expect(flash.alert).to eq("Could not perform action. User doesnt have any notifications configured.")
    end
