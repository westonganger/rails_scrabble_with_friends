  ...
  class GamesController < ApplicationController
    ...
    def send_turn_reminder
      ...
      if game_current_player
        notification_message = "#{current_user_player.username} sent your a reminder for you to play your turn on your Scrabble with Friends game."
        ...
        if subscriptions.any?
          ...
          flash[:alert] = "Could not perform action. User doesnt have any notifications configured."
        end
        redirect_to(action: :show)
      else
    ...
    def create_web_push_subscription
      ...
      push_subscription_params = params.require(:push_subscription).permit(:endpoint, :p256dh, :auth)

      subscription = current_user_player.web_push_subscriptions
        .where(game_id: @game.id)
        .find_by(push_subscription_params)

      if subscription
        subscription.touch
      else
        push_subscription_params[:game_id] = @game.id
        push_subscription_params[:user_agent] = request.user_agent, # nice to have field, serves to actual purpose other than debugging support

        current_user_player.web_push_subscriptions.create!(push_subscription_params)
      end
    ...
    def _send_game_over_notifications!
      ...
      notification_message = "#{winning_player_username} has won your Scrabble with Friends game"
    ...
    def _send_its_your_turn_notifications!(player)
      notification_message = "Its your turn to play on your Scrabble with Friends game"
